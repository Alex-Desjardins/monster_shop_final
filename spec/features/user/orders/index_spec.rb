...
RSpec.describe 'User Order Show Page' do
  describe 'As a Registered User' do
    before :each do
      ...
      @order_1.order_items.create!(item: @ogre, price: @ogre.price, quantity: 2) #40
      @order_2.order_items.create!(item: @giant, price: @giant.price, quantity: 2) #100
      @order_2.order_items.create!(item: @ogre, price: @ogre.price, quantity: 2) #40
    ...
    it 'I see order total changen when items have discounts' do
      ...
      within "#order-#{@order_1.id}" do
        expect(page).to have_content("Total: $38.00")
      end

      within "#order-#{@order_2.id}" do
        expect(page).to have_content("Total: $133.00")
      end
    end
  end
end
